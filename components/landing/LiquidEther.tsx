...
const defaultColors = ['#0066FF', '#0040CC', '#001F66'];
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      listenerTarget: Window | null = null; isHoverInside = false; hasUserControl = false;
    ...
    class ShaderPass {
      props: Record<string, unknown>; uniforms?: Uniforms;
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps?: Record<string, unknown>) {
        ...
        const cursorSize = (simProps?.cursor_size as number) ?? 100;
        const mouseM = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false, uniforms: { px: { value: simProps?.cellScale }, force: { value: new THREE.Vector2(0, 0) }, center: { value: new THREE.Vector2(0, 0) }, scale: { value: new THREE.Vector2(cursorSize, cursorSize) } } });
    ...
    class Viscous extends ShaderPass {
      ...
      update(args: { viscous?: number; iterations?: number; dt?: number }): THREE.WebGLRenderTarget | undefined {
        if (!this.uniforms) return;
    ...
    class Divergence extends ShaderPass {
      ...
      update(args: { vel?: THREE.WebGLRenderTarget }) { if (this.uniforms && args.vel) this.uniforms.velocity.value = args.vel.texture; super.update(); }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(args: { iterations?: number }): THREE.WebGLRenderTarget | undefined {
    ...
    class Simulation {
      ...
      update() {
        ...
        this.externalForce.update({ cursor_size: this.options.cursor_size, mouse_force: this.options.mouse_force, cellScale: this.cellScale } as Record<string, unknown>);
